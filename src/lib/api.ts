...
export const fetchCharacterList = () => {
  return fetch(
    `${BASE_URL}?ts=1&apikey=${PUBLIC_KEY}&hash=${hash}&limit=${50}`
...
export const fetchQueryCharacter = (query: string) => {
  console.log(query);
  return fetch(
    `${BASE_URL}?nameStartsWith=${query}&ts=1&apikey=${PUBLIC_KEY}&hash=${hash}`
  ).then((response) => response.json());
};

export const fetchCharacterItem = (id: string) => {
  return fetch(
    `${BASE_URL}/${id}?ts=1&apikey=${PUBLIC_KEY}&hash=${hash}`
  ).then((response) => response.json());
};
